...
device_counts = df[['Device 0', 'Device 1', 'Device 2', 'Device 3', 'Device 4', 'Device 5', 'Device 6', 'Device 7', 'Device 8']].apply(pd.Series.value_counts)
display(device_counts)
# Count the number of devices in each column and sum them up
device_counts = df[['Device 0', 'Device 1', 'Device 2', 'Device 3', 'Device 4', 'Device 5', 'Device 6', 'Device 7', 'Device 8']].stack().value_counts()
display(device_counts)
...
# Create a bar graph
top_devices.plot(kind='bar', color=plt.cm.tab20(range(len(top_devices))))
#device_counts.plot(kind='bar')
plt.xlabel('Devices')
plt.ylabel(' Device Count')
plt.title('Common Devices')
labels = top_devices.index
colors = [plt.cm.tab20(i) for i in range(len(top_devices))]
legend_patches = [plt.Rectangle((0,0),1,1,fc=plt.cm.tab20(i), edgecolor = 'none') for i in range(len(top_devices))]
...
df['hiking_duration_numeric'] = df['hiking_duration'].replace(mapping)
...
df.loc[df['Smart Watch Found'], 'Smart Watch Found'] = 'Smart watch'

# Replace False with 'Not smart watch' in the 'Smart Watch Found' column
df.loc[df['Smart Watch Found'].eq(False), 'Smart Watch Found'] = 'Not smart watch'
...
df['like_to_hike_alone_num'] = df['like_to_hike_alone'].replace(mapping)
df['like_to_hike_alone_num'] = df['like_to_hike_alone_num'].astype(int)
print ("Done")
#exclude the rows with fill in the blank in Gender column
df_valid_gender = df[df['Gender'] == 'Fill in the blank']
# create a pivot table of the like_to_hike_alone_num
print('\nlike_to_hike_alone_num')
pivot_table_like_to_hike_alone_num = df_valid_gender.pivot_table(index='Gender', columns='like_to_hike_alone_num', aggfunc='size', fill_value=0)
print(pivot_table_like_to_hike_alone_num)

  # Filter the data to only include male and female rows with a value in the like_to_hike_alone_num column
df_gender_like_to_hike_alone_num = df[df['Gender'].isin(['Male', 'Female']) & df['like_to_hike_alone_num'].notnull()][['Gender', 'like_to_hike_alone_num']]

# Perform the t-test
t_stat, p_value = ttest_ind(df_gender_like_to_hike_alone_num[df_gender_like_to_hike_alone_num['Gender'] == 'Male']['like_to_hike_alone_num'],
                            df_gender_like_to_hike_alone_num[df_gender_like_to_hike_alone_num['Gender'] == 'Female']['like_to_hike_alone_num'],
                            equal_var=False)
...
# Determine if the result is statistically significant
if p_value < alpha:
    print("**Statistically significant difference** between male and female participants' like_to_hike_alone_num values.")
    print(f"The mean like_to_hike_alone_num value for male participants is {df_gender_like_to_hike_alone_num[df_gender_like_to_hike_alone_num['Gender'] == 'Male']['like_to_hike_alone_num'].mean():.2f}.")
    print(f"The mean like_to_hike_alone_num value for female participants is {df_gender_like_to_hike_alone_num[df_gender_like_to_hike_alone_num['Gender'] == 'Female']['like_to_hike_alone_num'].mean():.2f}.")
else:
    print("No statistically significant difference between male and female participants' like_to_hike_alone_num values.")
print('\n')
# Group by Gender and calculate summary statistics
summary_stats = df_valid_gender.groupby('Gender')['like_to_hike_alone_num'].describe()
display(summary_stats)
# Create a bar graph of the like_to_hike_in_group_num column for male and female participants
df_gender_like_to_hike_alone_num.boxplot(column='like_to_hike_alone_num', by='Gender')
plt.xlabel('Gender')
...
df['like_to_hike_near_home_num'] = df['like_to_hike_near_home'].replace(mapping)
...
df['hike_while_traveling_num'] = df['hike_while_traveling'].replace(mapping)
...
df['hike_for_social_interaction_num'] = df['hike_for_social_interaction'].replace(mapping)
...
df['easy_hike_num'] = df['easy_hike'].replace(mapping)
...
print('\neasy_hike_num table\n')
pivot_table_easy_hike_num = df_valid_gender.pivot_table(index='Gender', columns='easy_hike_num', aggfunc='size', fill_value=0)
print(pivot_table_easy_hike_num)
print('Done\n')
# Set significance level
alpha = 0.05
# Filter the male and female rows with a value in the easy_hike_num column
df_gender_easy_hike_num = df[df['Gender'].isin(['Male', 'Female']) & df['easy_hike_num'].notnull()][['Gender', 'easy_hike_num']]
# Perform the t-test
t_stat, p_value = ttest_ind(df_gender_easy_hike_num[df_gender_easy_hike_num['Gender'] == 'Male']['easy_hike_num'],
                            df_gender_easy_hike_num[df_gender_easy_hike_num['Gender'] == 'Female']['easy_hike_num'],
                            equal_var=False)
# Print the t-statistic and p-value
print(f"t-statistic: {t_stat:.2f}")
print(f"p-value: {p_value:.4f}")
print('\n')
# Determine if the result is statistically significant between male and female participants' easy hike values
is_significant = p_value < alpha
print(f"Is it statistically significant at the {alpha} level? {is_significant}")
print('\n')
# Determine if the result is statistically significant
if p_value < alpha:
    print("**Statistically significant difference** between male and female participants' easy hike values.")
    print(f"The mean for male participants is {df_gender_easy_hike_num[df_gender_easy_hike_num['Gender'] == 'Male']['easy_hike_num'].mean():.2f}.")
    print(f"The mean for female participants is {df_gender_easy_hike_num[df_gender_easy_hike_num['Gender'] == 'Female']['easy_hike_num'].mean():.2f}.")
    print('\n')
else:
    print("No statistically significant difference between male and female participants' for easy hike.\n")
# Group by Gender and calculate summary statistics
summary_stats = df_valid_gender.groupby('Gender')['easy_hike_num'].describe()
display(summary_stats)
# Create a bar graph of the easy_hike_num column
df_gender_easy_hike_num.boxplot(column='easy_hike_num', by='Gender')
plt.xlabel('Gender')
plt.ylabel('Easy Hike (numeric)')
plt.title('Easy Hike')
plt.show()
print('done\n')


# Define a mapping from categories to numerical values of difficult hike column
mapping = {
    'Strongly disagree': 1,  # 1 for strongly disagree
    'Somewhat disagree': 2,  # 2 for somewhat disagree
    'Neither agree nor disagree': 3, # 3 for neither agree nor disagree
...
print ("\nreplacement of difficult hike to numeric value...")
df['difficult_hike_n'] = df['difficult_hike'].replace(mapping)
df['difficult_hike_n'] = df['difficult_hike_n'].astype(int)
print ("\n")
#exclude the rows with fill in the blank
df_valid_gender = df[df['Gender'] != 'Fill in the blank']
# create a pivot table of the difficult_hike_n
#test pivot
...
variables = ['like_to_hike_alone_num', 'like_to_hike_in_group_num', 'like_to_hike_near_home_num', 'hike_while_traveling_num', 'hike_for_health_num', 'hike_for_social_interaction_num', 'hike_for_fun_num', 'hike_for_meditation_num', 'hike_for_less_than_1hour_num', 'hike_for_half_a_day_num', 'hike_for_less_than_a_day_num', 'hike_for_multiple_days_num', 'easy_hike_num', 'difficult_hike_n']
...
print(pivot_table_female_2_devices)
...
df.loc[df['Headphones'], 'Headphones'] = 'Headphones'  

# Replace False with 'No Headphones' in the 'Headphones' column
df.loc[df['Headphones'].eq(False), 'Headphones'] = 'No Headphones'
